        ...
        try {
            _conn.dbContext((error, db) => {
                ...
                categories.findOne({_id: ObjectId(id)}, (err, result) => {
                    if (err) throw err;
                    if (result) {
                        return res.status(200).send({ code: 200, message: `Read Successfully`, data: result });
                    }
                    else {
                        return res.status(404).send({ code: 404, message: `Not Found` });
                    }
            ...
            _conn.dbContext((error, db) => {
                ...
                categories.insertOne({name: name}, (err, result) => {
                    if (err) throw err;
                        ...
                        return res.status(200).send({ code: 200, message: "Category successfully created" });
                    }
                });
            });
        } catch (error) {
                    ...
                    if (data) {
                        ...
                        (ero, result) => {
                            ...
                            if (result.modifiedCount == 0) {
                                return res.status(500).send({ code: 500, message: `Update Category failed` });
                            }
                            else {
            ...
            _conn.dbContext((error, db) => {
                ...
                categories.deleteOne({_id: ObjectId(id)}, (error, result) => {
                    ...
                    if (result.deletedCount == 0) {
                        return res.status(500).send({ code: 500, message: `Delete category failed` });
                    }
                    return res.status(200).send({ code: 200, message: `Category has deleted` });
                });
            });
        } catch (error) {
            return res.status(500).send({ code: 500, message: `Internal Server Error: ${error}` });
